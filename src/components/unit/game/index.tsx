      ...
      <div className="flex flex-wrap justify-center gap-3 w-full mt-8 pb-20 lg:mb-0">
        {GameList.map((el, idx) => (
          <Link
            ...
            key={`${el.name}_${idx}`}
            className={`
                relative w-1/3 h-40 rounded-2xl transition-colors duration-200 group
            ...
            {el.available ? (
              <span
                className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-0 
                  group-hover:opacity-100 transition-opacity duration-200 text-xl break-keep"
              >
                {el.name}
              </span>
            ) : (
              <span className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  text-xl break-keep">{el.name}</span>
            )}
          </Link>
        ))}
      </div>
